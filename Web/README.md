...
Deploy initially with both reminder channels disabled. Verify a pilot employee's encrypted `EmailAdd` and `EmailPass` and plain-text `EmailTo`, then enable email for that controlled test. The SMS API `RECEIVER` is the slash-joined pair `employeeUsername/approverUsername` (matching the legacy desktop `FILE` notification) with the ERF employee username as `SENDER`, posted as the same JSON payload the desktop SMS client sends; enable SMS only after a controlled staging request succeeds. HTTP 200 is recorded as `Sent`, meaning the API accepted the request; it does not claim carrier delivery acknowledgement. When an employee has a plain-text `NotificationEmail` ("Reminder Email" in desktop Account Settings), employee-audience reminder email goes to that address instead of the decrypted `EmailAdd`; an unparseable configured value is recorded as `NOTIFICATION_EMAIL_INVALID` rather than silently falling back.
