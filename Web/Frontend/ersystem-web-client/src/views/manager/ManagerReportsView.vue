  ...
  <section class="queue-page-shell">
    <AppStickyQueueControls>
      ...
      >
        <AppRefreshButton
          :loading="loading"
          accessible-label="Refresh Manager approvals"
      ...
      <AppFilterBar mobile-title="Approval filters">
        <template #primary>
          <v-text-field
            ...
            label="Employee or ERF reference"
    ...
    >
      <template #item.erfReferenceNumber="{ item }">
        <span class="queue-reference">{{ item.erfReferenceNumber || 'Not assigned' }}</span>
      </template>
            ...
            <div class="d-flex align-start justify-space-between ga-3">
              <div>
                ...
                <div class="text-body-2 muted">
                  {{ (item as ManagerReportListItem).erfReferenceNumber || 'ERF reference not assigned' }} · {{ (item as ManagerReportListItem).department || 'No department' }}
                </div>
              </div>
